...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    !python3 dkit.py train --dataset=../datasets/IKEA_FIXA --weights=imagenet
...
class DkitDataset(utils.Dataset):

    def load_dkit(self, dataset_dir, subset):
        """Load a subset of the Dkit dataset.
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        """
        # If not a dkit dataset image, delegate to parent class.
        image_info = self.image_info[image_id]
        # print (image_info["source"])
        if image_info["source"] != "dkit":
        ...
        for i, p in enumerate(info["polygons"]):
            ...
            try:
                ...
                print("Fix "+self.image_info[image_id]["path"])
...
def train(model):
    """Train the model."""
    # Training dataset.
    dataset_train = DkitDataset()
    dataset_train.load_dkit(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = DkitDataset()
    dataset_val.load_dkit(args.dataset, "val")
    dataset_val.prepare()
